...
class NeuroTN:
    ...
    def network_contraction_grads(self, target, label, optimize='dp', verbose=False):
        ...
        if self.network_contraction(target, optimize, False):
            ...
            def expr_with_mse_loss(W, B, target, label):
                ...
                mse_loss = jnp.sum(mse_loss) # Note: here we use sum instead of mean in the task of tensor factorization
    ...
    def ABEG(self, learning_rate, grad_cores, ab=(1.0, 0.0)):
        grad_W, grad_B = grad_cores
        alpha, beta = ab
        ...
        if self.core_mode == 2:
            for idx, (t, gW) in enumerate(zip(self.trainable_list, grad_W)):
                if t:
                    # self.W[idx] -= learning_rate * gW
                    self.W[idx] = self.W[idx] * exp_tsallis((-learning_rate/self.W[idx] ** (alpha+beta-1)) * gW, beta)
        else:
            for idx, (t, gW, gB) in enumerate(zip(self.trainable_list, grad_W, grad_B)):
                if t:
                    # self.W[idx] -= learning_rate * gW
                    # self.B[idx] -= learning_rate * gB
                    self.W[idx] = self.W[idx] * exp_tsallis((-learning_rate/self.W[idx] ** (alpha+beta-1)) * gW, beta)
                    self.B[idx] = self.B[idx] * exp_tsallis((-learning_rate/self.B[idx] ** (alpha+beta-1)) * gB, beta)
    
    def ABEG_iteration(self, learning_rate, target, label, ab=(1.0, 0.0), optimize='dp', verbose=False):
        loss, grad_cores = self.network_contraction_grads(target, label, optimize, verbose)
        self.ABEG(learning_rate, grad_cores, ab)
